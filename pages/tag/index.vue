<template>
  <div class="page-tag-all">
    <div class="main-layout">
      <Header type="tags" search placeholder="Product Tags" @resultSearch="getResultSearch" />
        ...
        <div v-for="(keyword, index) in items" :key="index" class="mb-6">
          ...
          <div class="flex flex-wrap items-center -mx-2">
            <div
              ...
              :key="itemKey"
              class="product-tag-card-wrap px-2 mt-2"
            >
              <ProductCardTag :item="item.name" />
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    itemResult() {
      return this.$_.filter(this.productTags, (i) => {
